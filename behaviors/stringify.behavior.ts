...
import { description, expectedDescription, expectedValue, problem } from "../src/matcher";
...
export default behavior("stringify", [
  ...
  exhibit("stringify an expected value", () => {
    return stringify(expectedValue(17, "is equal to"), testFormatter)
  }).check([
    property("it prints the representation with proper formatting", (result) => {
      assert.deepEqual(result, "info(a number that is equal to 17)")
    })
  ]),

  exhibit("stringify an expected description", () => {
    return stringify(expectedDescription("a string", "contains", description("something")), testFormatter)
  }).check([
    property("it prints the representation with proper formatting", (result) => {
      assert.deepEqual(result, "info(a string that contains something)")
    })
  ])
